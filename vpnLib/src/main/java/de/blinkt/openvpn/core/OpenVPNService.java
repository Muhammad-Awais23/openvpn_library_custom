...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    private void checkAndResumeTimerMonitoring() {
        try {
            ...
            if (!isProUser && allowedDuration > 0 && startTime > 0) {
                ...
                if (currentStatus != null && currentStatus.equalsIgnoreCase("connected")) {
                    Log.d(TAG, "✅ VPN is connected, resuming timer monitoring");
                    ...
                    if (elapsedSeconds >= allowedDuration) {
                        Log.d(TAG, "⚠️ Time already expired, disconnecting immediately");
                        disconnectDueToTimeLimit();
                    } else {
                        Log.d(TAG, "⏱️ Starting timer monitoring, remaining: " +
                                (allowedDuration - elapsedSeconds) + " seconds");
                        startTimerMonitoring();
                    }
                } else {
                    Log.d(TAG, "⚠️ VPN not connected (status: " + currentStatus +
                            "), skipping timer resume");
                }
            } else {
                Log.d(TAG, "⚠️ Timer settings not valid for resuming");
            }
        } catch (Exception e) {
    ...
    public void endVpnService() {
        ...
        // Only stop self if notification should not persist
        if (!mNotificationAlwaysVisible) {
            try {
                stopSelf();
                VpnStatus.removeStateListener(this);
                Log.d(TAG, "✅ Called stopSelf()");
            } catch (Exception e) {
                Log.e(TAG, "Error in stopSelf: " + e.getMessage());
            }
        }
...
//            });
    ...
    private void forceCleanState() {
        ...
        Log.d(TAG, "✅ State cleaned - Ready for new connection");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && "FORCE_DISCONNECT".equals(intent.getAction())) {
            Log.d(TAG, "🛑 FORCE_DISCONNECT received");
            disconnectDueToTimeLimit();
        ...
        if (intent != null && "UPDATE_TIMER".equals(intent.getAction())) {
            ...
            boolean vpnActuallyConnected = isVpnConnected &&
                    (currentStatus != null && currentStatus.equalsIgnoreCase("connected"));
            ...
            if (isProUser) {
                ...
                if (vpnActuallyConnected) {
                    ...
                    // Wait a moment for cleanup, then start fresh timer
        ...
        if (intent != null && "START_TIMER_MONITORING".equals(intent.getAction())) {
            ...
            if (durationSeconds > 0 || isProUser) {
                ...
                Log.d(TAG, "✅ Timer preferences saved, will start when VPN connects");
            }

            return START_STICKY;
        }

        // Handle always show notification
        ...
        if (intent != null && DISCONNECT_VPN.equals(intent.getAction())) {
            Log.d(TAG, "🛑 DISCONNECT_VPN action received");
            try {
                stopVPN(false);
        ...
        if (intent != null && PAUSE_VPN.equals(intent.getAction())) {
            Log.d(TAG, "⏸️ PAUSE_VPN action received");
            if (mDeviceStateReceiver != null)
        ...
        if (intent != null && RESUME_VPN.equals(intent.getAction())) {
            Log.d(TAG, "▶️ RESUME_VPN action received");
            if (mDeviceStateReceiver != null)
        ...
        if (intent != null && START_SERVICE.equals(intent.getAction())) {
            Log.d(TAG, "🚀 START_SERVICE action received");
            return START_NOT_STICKY;
        }

        if (intent != null && START_SERVICE_STICKY.equals(intent.getAction())) {
            Log.d(TAG, "🚀 START_SERVICE_STICKY action received");
        ...
        if (intent != null && intent.hasExtra(getPackageName() + ".profileUUID")) {
            ...
            Log.d(TAG, "📋 Profile UUID from intent: " + profileUUID);
            ...
            Log.d(TAG, "📋 No profile UUID in intent, getting last connected profile");
            ...
            if (mProfile == null) {
                ...
                if (mProfile == null) {
                    Log.e(TAG, "❌ No profile found, stopping service");
                    stopSelf(startId);
                    return START_NOT_STICKY;
                }
            }
        ...
        if (mProfile == null) {
            Log.e(TAG, "❌ Profile is null, stopping service");
            stopSelf(startId);
            return START_NOT_STICKY;
        }

        Log.d(TAG, "✅ Profile loaded: " + mProfile.mName);
        ...
        // Check and resume timer monitoring after connection establishes
        new Handler(getMainLooper()).postDelayed(() -> {
            Log.d(TAG, "⏰ Checking if timer monitoring should resume...");
    ...
    private void startOpenVPN() {
        ...
        // ✅ CRITICAL: Follow DEFAULT library pattern exactly
        ...
        // ✅ CRITICAL: An old running VPN should now be exited
        // Reset the flag AFTER cleanup like default library
        mStarting = false;
    ...
    private void checkVpnTimeLimit() {
        try {
            Log.d(TAG, "⏰ checkVpnTimeLimit - isVpnConnected=" + isVpnConnected);
            ...
            // ✅ Double check with status string (case insensitive)
            String currentStatus = OpenVPNService.getStatus();
            if (currentStatus == null || !currentStatus.equalsIgnoreCase("connected")) {
                Log.d(TAG, "❌ VPN status not connected: " + currentStatus);
                stopTimerMonitoring();
                return;
            }
    ...
    private void startTimerMonitoring() {
        try {
            Log.d(TAG, "🔄 ========== startTimerMonitoring CALLED ==========");

            // ✅ Check if VPN is actually connected
            if (!isVpnConnected) {
                Log.d(TAG, "❌ VPN not connected (isVpnConnected=false), skipping timer start");
                return;
            }
            ...
            if (currentStatus == null || !currentStatus.equalsIgnoreCase("connected")) {
                Log.d(TAG, "❌ VPN status not connected, skipping timer start");
                return;
            }
            ...
            if (isProUser || allowedDuration <= 0) {
                Log.d(TAG, "⚠️ Timer not needed - Pro user or no duration set");
                return;
            }

            long startTime = prefs.getLong(KEY_CONNECTION_START_TIME, 0);

            // ✅ IMPORTANT: If no start time, set it NOW
            if (startTime == 0) {
                long now = System.currentTimeMillis();
                prefs.edit().putLong(KEY_CONNECTION_START_TIME, now).commit();
            ...
            Log.d(TAG, "   - Total duration: " + allowedDuration + "s");
            Log.d(TAG, "   - Elapsed: " + elapsedSeconds + "s");
            ...
            if (isTimerMonitoringActive) {
                Log.d(TAG, "⚠️ Stopping existing timer before starting new one");
                stopTimerMonitoring();
            }
            ...
            Log.d(TAG, "========== startTimerMonitoring END ==========");
    ...
    public void updateState(String state, String logmessage, int resid,
        ...
        Log.d(TAG, "📊 updateState: " + state + " -> " + level);
        ...
        if (level == LEVEL_CONNECTED) {
            ...
            new Handler(Looper.getMainLooper()).postDelayed(() -> {
                if (isVpnConnected) {
                    ...
                    if (!isProUser && allowedDuration > 0) {
                        Log.d(TAG, "✅ Starting timer monitoring");
                        startTimerMonitoring();
                    }
                }
            }, 2000);
